...
def wyszukaj_ksiazki():
    ...
    if choice == '1':
        ...
        try:
            ...
            mycursor.execute(query)
            print(dash)
            print('{:<4s}'.format('ID'), '{:<10s}'.format('ID Autora'), '{:<15s}'.format('WYDAWNICTWO'),
                  '{:<10s}'.format('GATUNEK'), '{:<15s}'.format('TYTUŁ'))
            print(dash)
            for x, y, z, a, b in mycursor:
                print('{:<4d}'.format(x), '{:<10d}'.format(y), '{:<15s}'.format(z), '{:<10s}'.format(a),
                      '{:<15s}'.format(b))
            print(dash)
            input('Press any key to continue...')
        except:
